...
with st.sidebar:
    ...
    if user_prompt:
        ...
        with st.spinner('Working on your requests....'):
            responese = chat(st.session_state.messages)

        st.session_state.messages.append(
            AIMessage(content=responese.content)
        )
#st.session_state.messages

# message('This is chat gpt', is_user=False)
...
for i,msg in enumerate(st.session_state.messages[1:]):
    if i % 2 == 0:
        message(msg.content, is_user=True, key = f'{i} + :penguin:')
    else:
